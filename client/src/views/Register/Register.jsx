  ...
  <form className={style.form} onSubmit={(e)=>handleSubmit(e)}>
    <div className={style.inputName}>
      ...
      <div className={style.titleContraseña}>
          ...
          onChange={(e)=>handleChange(e)}
          />
          {errors.contraseña && <div className={style.error}>{errors.contraseña}</div>}
        </div>
        <div className={style.title3}>
          <h3>Recibiras confirmación del registro por email</h3>
        </div >
        <div className={style.title4}>
        <button className={style.button} type= 'submit'>Registrarme</button>
        </div>
    ...
    <Link className= "LinkForm" to= '/'><button className={style.button2}>Go Home</button></Link>
    </div>
   </div>
  )
}
